 ...
 *
 * Algorithm:
 *   1. Count k-mers on both strands of the input sequence.
 *   2. Retain k-mers observed ≥ MIN_COUNT times (= repeated).
 *   3. Mark per-base coverage by repeated k-mers.
 *   4. Extract contiguous blocks with sufficient coverage and length.
 */
public class MaskingSequence {
     ...
     * it is counted as a "high-coverage" base that anchors a repeat block.
     * (set to 1 so that coverage > 1 means ≥ 2 overlapping k-mers)
     */
    ...
    public long gapsLength()  { return gapsLen;    }
    public long repeatLength() { return repeatsLen; }
    ...
    private void countStrand(LongIntHashMap map, byte[] bases, int kmer) {
        ...
        for (int i = kmer - 1; i < len; i++) {
            ...
            if (gapCount == 0) {
                ...
                // At MIN_COUNT the entry is already "confirmed repeat"; no need
                // to increment further.
            }

            // Slide the window: drop the base leaving the left end.
            if (bases[i - kmer + 1] == 4) gapCount--;
        }
    }
     ...
     * increment coverage for every base it spans (unless the base is SSR-masked).
     */
    private void markStrandForward(LongIntHashMap kmerMap,
                                   byte[] bases, byte[] ssrmsk,
                                   int[] coverage, int kmer) {
        final int len = bases.length;
        int gapCount = 0;
        for (int i = 0; i < kmer - 1; i++) {
            if (bases[i] == 4) gapCount++;
            ...
            if (gapCount == 0) {
                ...
                if (kmerMap.get(code) >= MIN_COUNT) {
                    for (int j = start; j <= i; j++) {
                        if (ssrmsk[j] == 0) coverage[j]++;
                    }
                }
            }

            if (bases[start] == 4) gapCount--;
        }
    }
     ...
     */
    private void markStrandReverse(LongIntHashMap kmerMap,
                                   byte[] bases, byte[] ssrmsk,
                                   int[] coverage, int len, int kmer) {
        int gapCount = 0;
        for (int i = 0; i < kmer - 1; i++) {
            if (bases[i] == 4) gapCount++;
        }
     ...
     * block is retained only when at least 2 positions exceed {@code top}
     * (the half-kmer or full-kmer threshold), which guards against very short
     ...
     *
     * Alphabet mapping (from {@code tables.dx2}):
     *   A=0, C=1, G=2, T=3, N(gap)=4
     *
     * Using base 5 (one digit per base):
     *   - Clean: no ambiguity, no hash collision from different k-mers.
     *   - Safe range: 5^27 = 7.45×10^18 < Long.MAX_VALUE (9.22×10^18),
     *     so k-mers up to length 27 always produce non-negative codes.
     *   - Codes of gap-containing k-mers are never inserted (gapCount guard),
     *     so digit value 4 appears only in invalid/unreachable codes.
     *
     * NOTE: Base-5 codes cannot equal EMPTY_KEY (Long.MIN_VALUE) or
     * DELETED_KEY (Long.MIN_VALUE+1) for k ≤ 27 because those sentinels are
     * negative and base-5 codes are non-negative in that range.
     *
     * Replacing the old base-10 encoding:
     *   - Base 10 wasted ~1.3 bits per base.
     *   - Base 10 limited safe (non-negative) k-mer length to 19.
     *   - Base 5 raises that limit to 27, sufficient for typical repeat masking.
     */
